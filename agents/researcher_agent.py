...
from tools.tools_coder_pipeline import prepare_list_dir_tool, prepare_see_file_tool, retrieve_files_by_semantic_query
...
if anthropic_api_key:
    llms.append(
        ChatAnthropic(model="claude-3-5-sonnet-20241022", temperature=0.2, timeout=60).with_config(
            {"run_name": "Researcher"},
        ),
    )
...
with open(f"{parent_dir}/prompts/researcher_system.prompt") as f:
...
def call_model_researcher(state):
    ...
    if hasattr(last_message, "json5_tool_calls") and len(last_message.json5_tool_calls) > 1:
        ...
        state["messages"][-1].json5_tool_calls = [
            tool_call
            for tool_call in last_message.json5_tool_calls
...
def after_agent_condition(state):
    ...
    if last_message.content in (bad_json_format_msg, no_json_msg, finish_too_early_msg):
        return "agent"
    if last_message.json5_tool_calls[0]["tool"] == "final_response_researcher":
        return "human"
    return "tool"
